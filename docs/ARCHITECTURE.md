...
Le frontend ne parle jamais directement au backend : `next.config.ts` proxifie `/api/*` vers `BACKEND_URL` (`rewrites()`, défaut `http://localhost:8000`), donc dans le navigateur tout part vers l'origine du frontend sous `/api/...`. C'est pour ça qu'il n'y a pas de souci CORS en usage normal — CORS ne sert qu'aux tests/scripts qui appellent le backend en direct (`ALLOWED_ORIGINS` dans `main.py`, liste séparée par des virgules, défaut `http://localhost:3000` — à positionner sur l'URL Vercel en production).
...
1. **Upload** — `POST /api/documents` : le PDF part en `multipart/form-data` (rejeté au-delà de `MAX_UPLOAD_MB`, 20 Mo par défaut), le backend le garde en mémoire (`documents.py`), renvoie un `document_id` (uuid). Le fichier n'est jamais écrit sur disque, ni côté client ni côté serveur.
