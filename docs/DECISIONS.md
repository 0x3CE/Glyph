...
**Non portable, résolu avant le déploiement** : ces chemins sont spécifiques à macOS — sur Render (Linux), `_SYSTEM_FONT_DIR` n'existe simplement pas. `_system_font_path` essaie maintenant un second répertoire, `_BUNDLED_FONT_DIR` (`backend/app/fonts/liberation/`), qui embarque les polices Liberation Sans/Serif/Mono (licence SIL OFL, libre de redistribution) — métriquement identiques à Arial/Times New Roman/Courier New (même chasse de caractère, glyphe par glyphe), donc même en dernier recours le texte reflow comme l'aurait fait la vraie police. `georgia`/`verdana`/`tahoma` n'ont pas d'équivalent Liberation dédié et retombent sur la famille bundlée la plus proche (`verdana`/`tahoma` → Liberation Sans, `georgia` → Liberation Serif) — toujours strictement mieux que Base-14, qui reste le tout dernier recours si même les polices bundlées manquent (ne devrait jamais arriver, elles sont commit dans le repo). Validé par un test de bout en bout dans le vrai conteneur Docker Linux de déploiement (voir `backend/tests/test_pdf_engine.py::BundledFontFallbackTests`).
